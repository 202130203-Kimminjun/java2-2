...
## 6월 7일   
**paintComponent**  
ㆍ모든 컴포넌트는 자신의 모양을 스스로 그린다.  
ㆍ컨테이너는 자신을 그린 후 그 위에 자식 컴포넌트들에게 그리기 지시  
ㆍ모든 스윙 컴포넌트는 자신의 모양을 그리는 paintComponent() 메소드 보유  
...
**repaint**  
ㆍ모든 컴포넌트가 가지고 있는 메소드  
ㆍ자바 플랫폼에게 컴포넌트 그리기를 강제 지시하는 메소드  
ㆍrepaint()를 호출하면, 자바 플랫폼이 컴포넌트의 paintComponent() 호출  

**멀티 태스킹, 스레드와 운영체제**  
*Multi Tasking* - 여러 일을 동시에 하는 것.  
*스레드(thread)* - 운영체제에 의해 관리되는 하나의 작업 혹은 태스크.  
...
▶하나의 JVM은 하나의 자바 응용프로그램만 실행  
    └자바 응용프로그램이 시작될 때 JVM이 함께 실행  
    └자바 응용프로그램이 종료하면 JVM도 함께 종료  
▶응용프로그램은 하나 이상의 스레드로 구성 가능  

【스레드 만드는 2 가지 방법】  
ㆍjava.lang.Thread 클래스를 상속받아 스레드 작성  
ㆍjava.lang.Runnable 인터페이스를 구현하여 스레드 작성  
...
두 스레드가 하나의 프린터를 사용할 경우 충돌이 일어나 출력이 뒤죽박죽으로 나올 수도 있음.  
이를 방지하기 위해 동기화를 이용, 다른 스레드가 출력할 동안 다음 스레드는 대기.  
